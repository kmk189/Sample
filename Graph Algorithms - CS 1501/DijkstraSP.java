public class DijkstraSP {
    ...
    private EdgeInfo[] edgeTo;    // edgeTo[v] = last edge on shortest s->v path
     ...
     * @throws IllegalArgumentException unless {@code 0 <= v < V}
     */
    public Stack<EdgeInfo> pathTo(int v) {
        validateVertex(v);
        if (!hasPathTo(v)) return null;
        Stack<EdgeInfo> path = new Stack<EdgeInfo>();
        for (EdgeInfo e = edgeTo[v]; e != null; e = edgeTo[e.getVertex1()]) {
            path.push(e);
        }
        return path;
    }
    ...
    private boolean check(NetworkGraph G, int s) {
        ...
        for (int w = 0; w < G.V(); w++) {
            ...
            EdgeInfo e = edgeTo[w];
            int v = e.getVertex1();
            if (w != e.getVertex2()) return false;
            if (distTo[v] + e.getTime() != distTo[w]) {
