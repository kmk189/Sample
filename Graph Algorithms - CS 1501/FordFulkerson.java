public class FordFulkerson {
    ...
    private EdgeInfo[] edgeTo;    // edgeTo[v] = last edge on shortest residual s->v path
    ...
    public FordFulkerson(NetworkGraph G, int s, int t) {
        ...
        if (!isFeasible(G, s, t)){
			...
			System.out.println("Initial flow is infeasible");
		}
    ...
    private double excess(NetworkGraph G, int v) {
        double excess = 0.0;
        for (EdgeInfo e : G.adj(v)) {
            if (v == e.getVertex1()) excess -= e.flow();
            else               excess += e.flow();
        }
        return excess;
    }
    ...
    private boolean check(NetworkGraph G, int s, int t) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (EdgeInfo e : G.adj(v)) {
                if ((v == e.getVertex1()) && inCut(e.getVertex1()) && !inCut(e.getVertex2()))
